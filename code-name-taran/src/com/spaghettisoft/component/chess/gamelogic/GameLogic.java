...
public class GameLogic {
	...
	public GameLogic(Player whitePlayer, Player blackPlayer, ChessBoard chessBoard) {
	...
	public boolean move(Player player, Move move) throws EmtyFieldException, IncorectColorMoveException, IncorectColorCaptureException, IllegalMoveException, KingCheckException, IlligalCapturedKingException, IllegalMoveCheckException, IllegalEnPassantMoveException, SameFieldException, InvalidNameException, IllegalCastingMoveException {
		boolean result = false;
		if (canMove(player, move)) {
			AbstractFigure capturedFigure = this.chessBoard.getFigure(move.getToRow(), move.getToColumn());
			AbstractFigure movedFigure = this.chessBoard.getFigure(move.getFromRow(), move.getFromColumn());
			...
			this.chessBoard.addFigure(move.getToRow(), move.getToColumn(), this.chessBoard.getFigure(move.getFromRow(), move.getFromColumn()));
			this.chessBoard.addFigure(move.getFromRow(), move.getFromColumn(), null);
	...
	private boolean isCorrectColorFigureCurrentPosition(Player player, Move move) throws IncorectColorMoveException {
		Color figureColor =  this.chessBoard.getFigure(move.getFromRow(), move.getFromColumn()).getColor();
		Color playerColor =  player.getColor();
		boolean result = playerColor.equals(figureColor);
		return result;
	}  
	
	private boolean isCorrectColorFigureChoosenForMovingPosition(Player player, Move move) throws IncorectColorCaptureException {
		boolean result = false;
		...
		if (figure != null) {
			figureColor = this.chessBoard.getFigure(move.getToRow(), move.getToColumn()).getColor();
	...
	private boolean isFromFieldEmpty(int row, int column) throws EmtyFieldException {
		boolean result = (this.chessBoard.getFigure(row, column) == null);
		return result;
	}
	
	private boolean canPawnMove(Move move) throws EmtyFieldException {
		AbstractFigure figure = this.chessBoard.getFigure(move.getToRow(), move.getToColumn());
		if (isEnPassant(move)) {
			return true;
		} 
		...
		if ( (figure != null) && (Math.abs(move.getSubtractionbetweenColums()) == ONE) ) {
			return true;
		}
	...
	private boolean checkForFiguresBetweenTwoFields(Move move) {
		...
		do {
				...
				if (!IsRowAndColumAreMatrixRange(currentRow, currentColumn) || ((currentRow == move.getToRow()) && (currentColumn == move.getToColumn())) ) {
					break;
				}
	...
	private boolean isFieldInCheck(Player player, int row, int column) {
		...
		for (int i = 0; i < this.chessBoard.getBoardLenght(); i++) {
			for (int j = 0; j < this.chessBoard.getBoardLenght(); j++) {
				if (isThisFigureKnight(i, j) && isEnemyFigure(player, i, j)) {
					if(this.chessBoard.getFigure(i, j).validMove(new Move(i, j, row, column))) {
						return true;
					}
				}
			}
		...
		while (counter < directions.length) {
			figureCordinates = getPositionOfFirstFoundFigureInGivenDirection(player, row, column, directions[counter][ZERO], directions[counter][ONE]);

			if (figureCordinates != null) {
				figure = this.chessBoard.getFigure(figureCordinates[ZERO],figureCordinates[ONE]);
				move = new Move(figureCordinates[ZERO],figureCordinates[ONE], row, column);
				if ( isEnemyFigure(player, figureCordinates[ZERO], figureCordinates[ONE]) && figure.validMove(move) ){
					if( !(isThisFigurePawn(figureCordinates[ZERO], figureCordinates[ONE]) && (figureCordinates[ONE] == column)) ) {
						return true;
					} 
				}
			}
	...
	private int[] getPositionOfFirstFoundFigureInGivenDirection(Player player, int row, int column, int rowDirection, int columnDirection) {
		int currentRow = row;
		...
		while (true) {
				...
				figure = this.chessBoard.getFigure(currentRow, currentColumn);
				if ( figure!= null && !(!isEnemyFigure(player, currentRow, currentColumn) && isThisFigureKing(figure)) ) {
	...
	private boolean IsRowAndColumAreMatrixRange(int currentRow, int currentColum) {
		boolean result = (ZERO <= currentRow && currentRow < this.chessBoard.getBoardLenght()) && 
				      	 (ZERO <= currentColum && currentColum < this.chessBoard.getBoardLenght());
		return result;
	}
	...
	private boolean isKignInCheckWhenMoveFigure(Player player, Move move) {
		...
		Move moveBetweenKingAndMovedFigure = new Move(kingPosition[ZERO], kingPosition[ONE], move.getFromRow(), move.getFromColumn());
		AbstractFigure movedFigure = this.chessBoard.getFigure(move.getFromRow(), move.getFromColumn());
		AbstractFigure figure;
		...
		int[] directionsMoveBetweenKingAndMovedFigure = getDirection(moveBetweenKingAndMovedFigure);
		...
		if ( (directionRowCurrentMove == directionRowMoveBetweenKingAndMovedFigure) && (directionColumCurrentMove == directionColumMoveBetweenKingAndMovedFigure) ||
			 (directionRowCurrentMove * MINUS_ONE == directionRowMoveBetweenKingAndMovedFigure) && (directionColumCurrentMove * MINUS_ONE == directionColumMoveBetweenKingAndMovedFigure) ) {
			return false;
		}
		
		 boolean isSameRow = moveBetweenKingAndMovedFigure.getFromRow() == moveBetweenKingAndMovedFigure.getToRow();
		 boolean isSameColumn = moveBetweenKingAndMovedFigure.getFromColumn() == moveBetweenKingAndMovedFigure.getToColumn();
		 boolean isSameDiagonal = Math.abs(moveBetweenKingAndMovedFigure.getSubtractionbetweenRows()) == Math.abs(moveBetweenKingAndMovedFigure.getSubtractionbetweenColums());
		if ( player.getColor().equals(movedFigure.getColor()) && (isSameRow || isSameColumn || isSameDiagonal) ) {
			...
			do {
				...
				if ( (figure != null) && !figure.equals(movedFigure) ) {
					...
					result = figure.validMove(new Move(currentRow, currentColum, moveBetweenKingAndMovedFigure.getFromRow(), moveBetweenKingAndMovedFigure.getFromColumn()));
					break;
				}
	...
	private boolean isEnemyFigure(Player player, int row, int column) {
		boolean result =  !this.chessBoard.getFigure(row, column).getColor().equals(player.getColor());
		return result;
	}
	...
	private boolean isKingAgainstKing(AbstractFigure[] allLeftFigureOnChessBord){
		boolean result = false;
		if (allLeftFigureOnChessBord.length == TWO) {
			result = isThisFigureKing(allLeftFigureOnChessBord[ZERO]) && isThisFigureKing(allLeftFigureOnChessBord[ONE]);
		}
		return result;
	}
	
	private boolean isKingAgainstKingAndBishopOrKnight(AbstractFigure[] allLeftFigureOnChessBord) {
		boolean result = false;
		...
		if (allLeftFigureOnChessBord.length == THREE) {
			for (int i = 0; i < allLeftFigureOnChessBord.length; i++) {
				if (isThisFigureKing(allLeftFigureOnChessBord[i])) {
					numberOfKIngs++;
				} else if (isThisFigureBishop(allLeftFigureOnChessBord[i]) || isThisFigureKnight(allLeftFigureOnChessBord[i])) {
	...
	private boolean isKingAndBishopAgainstKingAndBishop(AbstractFigure[] allLeftFigureOnChessBord) {
		boolean result = false;
		int numberOfKIngs = 0;
		...
		if (allLeftFigureOnChessBord.length == FOUR) {
			...
			if (first != null && second != null) {
				...
				Color firstFoundBishop = getChessbordFieldColor(bishopsPosition[ZERO], bishopsPosition[ONE]);
				Color secondFoundBishop = getChessbordFieldColor(bishopsPosition[TWO], bishopsPosition[THREE]);
			result = (numberOfBishops == TWO) && firstFoundBishop.equals(secondFoundBishop) &&(numberOfKIngs == TWO);
			}
		}
		return result;
	}
	...
	private boolean isNoPossibilityOfCheckmate() {
		...
		boolean result = isKingAgainstKing(allLeftFigureOnChessBord) || 
						 ...
						 isKingAndBishopAgainstKingAndBishop(allLeftFigureOnChessBord);
		
		return result;
	}
	
	private boolean isFiftyMoveRule() {
		boolean result  =  (this.getNumberOfMovesWithoutCapture() == FIFTY_MOVES_RULE_VALUE) && (this.getNumberOfMovesWithoutMovePawn() == FIFTY_MOVES_RULE_VALUE);
		return result;
	}
	...
	private boolean isEnPassant(Move move) {
		...
		if (isThisFigurePawn(lastEnemyMove.getToRow(), lastEnemyMove.getToColumn()) && (Math.abs(lastEnemyMove.getSubtractionbetweenRows()) == TWO) ) {
			
			if ( isThisFigurePawn(currentRow, currentColumn) && (currentColumn!= (this.chessBoard.getBoardLenght()-ONE)) && isThisFigurePawn(currentRow, currentColumn + ONE)) {
				isOtherPawnCorrecColor = !( this.chessBoard.getFigure(currentRow, currentColumn).getColor().equals
									 (this.chessBoard.getFigure(currentRow, currentColumn + ONE).getColor()) );

				isPawnMovedCorrectSide = (moveColumn == (currentColumn + ONE));
				if (isOtherPawnCorrecColor && isPawnMovedCorrectSide) {
					
					if ( (currentRow == lastEnemyMove.getToRow()) && (currentColumn + ONE == lastEnemyMove.getToColumn()) ) {
						this.decrementNumberOfFicgureOnTheChessBoard();
						this.chessBoard.addFigure(currentRow, currentColumn + ONE, null);
						this.resetNumberOfMovesWithoutCapture();
			...
			if ( isThisFigurePawn(currentRow, currentColumn) && currentColumn!= ZERO && isThisFigurePawn(currentRow, currentColumn - ONE)) {
				isOtherPawnCorrecColor = !( this.chessBoard.getFigure(currentRow, currentColumn).getColor().equals
										 (this.chessBoard.getFigure(currentRow, currentColumn - ONE).getColor()) );
	...
	private boolean isCastling(Player player, Move move) {
		boolean result = false;
		AbstractFigure figureKing = this.chessBoard.getFigure(move.getFromRow(), move.getFromColumn());
		AbstractFigure rook;
		Move moveFromKingToRook;
		if(isThisFigureKing(move.getFromRow(), move.getFromColumn()) && figureKing.isFirstMove()) {
			int[] rookPosition = getRookPosition¿ccordingColorAndSide(figureKing, move);
			rook = this.chessBoard.getFigure(rookPosition[ZERO], rookPosition[ONE]);
			moveFromKingToRook = new Move(move.getFromRow(), move.getFromColumn(), rookPosition[ZERO], rookPosition[ONE]);;
			if (rook != null && rook.isFirstMove() && !isCurentPlayerInCheck() && !checkForFiguresBetweenTwoFields(moveFromKingToRook)) {
				int numberOfFieldrForCheck = Math.abs(move.getFromColumn() - rookPosition[ONE]);
				int dirrection;
				...
				for (int i = ONE; i < numberOfFieldrForCheck; i++) {
					...
					if (!isFieldInCheck(player, move.getFromRow(), currentColumn)) {
						numberFreeFields++;
					}
				}
				...
				if (result) {
					if (dirrection > ZERO){
						this.chessBoard.addFigure(move.getFromRow(), move.getFromColumn() + ONE, rook);
					} else {
						this.chessBoard.addFigure(move.getFromRow(), move.getFromColumn() - ONE, rook);
					}
					this.chessBoard.addFigure(rookPosition[ZERO], rookPosition[ONE], null);
				}
	...
	private int[] getRookPosition¿ccordingColorAndSide(AbstractFigure figure, Move move) {
	...
	private boolean canMove(Player player, Move move) throws EmtyFieldException, IncorectColorMoveException, IncorectColorCaptureException, IllegalMoveException, KingCheckException, IlligalCapturedKingException, IllegalMoveCheckException, IllegalEnPassantMoveException, SameFieldException, IllegalCastingMoveException {
		boolean result = true;
		...
		if (!this.chessBoard.getFigure(move.getFromRow(), move.getFromColumn()).validMove(move)) {
			throw new IllegalMoveException();
		}
		if ( !isThisFigureKnight(move.getFromRow(), move.getFromColumn()) &&  checkForFiguresBetweenTwoFields(move)) {
			throw new IllegalMoveException();
		}
		if (isThisFigurePawn(move.getFromRow(), move.getFromColumn()) && !canPawnMove(move)) {
			throw new IllegalMoveException();
		}
		if (isThisFigureKing(move.getFromRow(), move.getFromColumn()) && Math.abs(move.getSubtractionbetweenColums()) == TWO && !isCastling(player, move)) {
			throw new IllegalCastingMoveException();
		}
		if ( !isThisFigureKing(move.getFromRow(), move.getFromColumn()) && isKignInCheckWhenMoveFigure(player, move)) {
	...
	private void stopMoveFiguresIfKingIncheck(Player player, Move move)
		...
		if (isFieldInCheck(player, kingPosition[ZERO], kingPosition[ONE])) {
			this.chessBoard.addFigure(move.getFromRow(), move.getFromColumn(), this.chessBoard.getFigure(move.getToRow(), move.getToColumn()));
			this.chessBoard.addFigure(move.getToRow(), move.getToColumn(), null);
	...
	private void checkFoPawnPromotion(Player player, Move move) throws InvalidNameException {
		AbstractFigure figure = null;
		if (isThisFigurePawn(move.getToRow(), move.getToColumn())) {
			if ( this.chessBoard.getFigure(move.getToRow(), move.getToColumn()).getColor().equals(Color.WHITE)) {
				if ( move.getToRow() == ZERO) {
					if(move.getToColumn() % TWO == ZERO) {
						figure = choosePromotionFigure(player, move, Color.WHITE);
					} else {
						figure = choosePromotionFigure(player, move, Color.BLACK);
					}
					this.chessBoard.addFigure(move.getToRow(), move.getToColumn(), null);
					this.chessBoard.addFigure(move.getToRow(), move.getToColumn(), figure);
				}
			} else {
		...
		private AbstractFigure choosePromotionFigure(Player player, Move move, Color squareColor ) throws InvalidNameException {
			AbstractFigure figure = null;
			String userInput;
			
			while (true) {	
				Color playerColor = player.getColor();
				System.out.println(playerColor + " player choose figure to replace pawn.   Example: Bishop");
	...
	private boolean isSameField(Move move) {
		boolean result = move.getFromRow() == move.getToRow() && move.getToColumn() == move.getFromColumn();
		return result;
	}
	...
	private boolean isCheckmate(Player player) {
		int[] kingPosition = getCurrentPlayerKingPosition(player);
		boolean isAllFreeFieldsAroundKingAreInCheck = isAllFreeFieldsAroundKingAreInCheck(player, kingPosition);
		boolean can“he ingBeCoveredByFigure = can“he ingBeCoveredByFigure(player, kingPosition);
		return isAllFreeFieldsAroundKingAreInCheck && !can“he ingBeCoveredByFigure;
	}
	
	private boolean can“he ingBeCoveredByFigure(Player player, int[] kingPosition) {
		int counter = 0; 			
		AbstractFigure figure;
		int[] figureCordinates;
		...
		for (int i = 0; i < this.chessBoard.getBoardLenght(); i++) {
			for (int j = 0; j < this.chessBoard.getBoardLenght(); j++) {
				if (isThisFigureKnight(i, j) && isEnemyFigure(player, i, j)) {
					if(this.chessBoard.getFigure(i, j).validMove(new Move(i, j, kingPosition[ZERO], kingPosition[ONE]))) {
		...
		while (counter < directions.length) {
			figureCordinates = getPositionOfFirstFoundFigureInGivenDirection(player, kingPosition[ZERO], kingPosition[ONE], directions[counter][ZERO], directions[counter][ONE]);

			if (figureCordinates != null) {
				figure = this.chessBoard.getFigure(figureCordinates[ZERO],figureCordinates[ONE]);
				if (!figure.getColor().equals(player.getColor()) && figure.validMove(new Move(figureCordinates[ZERO],figureCordinates[ONE], kingPosition[ZERO], kingPosition[ONE]))) {
					if( !(isThisFigurePawn(figureCordinates[ZERO],figureCordinates[ONE]) && (figureCordinates[ONE] == kingPosition[ONE])) ) {
						if (numberOfFoundCordinates >= ONE) {
							return false;
						}
		...
		int [] direction = getDirection(new Move(cordinatesFigureAttackKing[ZERO],cordinatesFigureAttackKing[ONE], kingPosition[ZERO], kingPosition[ONE]));
		...
		while (true) {
			...
			for (int i = 0; i < this.chessBoard.getBoardLenght(); i++) {
				for (int j = 0; j < this.chessBoard.getBoardLenght(); j++) {
					...
					if (figure2 != null && !isEnemyFigure(player, i, j) && !isThisFigureKing(figure2)) {
						move = new Move(i, j, currentRow, currentColumn);
						if (isThisFigureKnight(i, j)) {
							if (figure2.validMove(move) && !isKignInCheckWhenMoveFigure(player, move)) {
								return true;
							}
						} else {
							if (figure2.validMove(move) && !checkForFiguresBetweenTwoFields(move) && !isKignInCheckWhenMoveFigure(player, move)) {
								return true;
							}
						}
					}
			...
			if ( !IsRowAndColumAreMatrixRange(currentRow, currentColumn) || (currentRow == kingPosition[ZERO] && currentColumn == kingPosition[ONE]) ) {
				break;	
			}
	...
	private boolean isAllFreeFieldsAroundKingAreInCheck(Player player , int[] kingPosition) {
		int row;
		int column;
		AbstractFigure figure;

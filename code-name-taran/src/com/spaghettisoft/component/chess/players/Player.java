...
public class Player {
    ...
    private boolean askOtherPlayerForRemy(Player player, String userInput) {
        ...
        while (true) {
            System.out.println(otherPlayerColor
                    + " player please answer with true or false");

            otherPlayerInput = StaticObjects.scanner.nextLine();
            String trimString = UserInputValidatorAndConvertor
                    .clearAllWhiteSpaces(otherPlayerInput);
            String toLowerCase = trimString.toLowerCase();

            if (toLowerCase.equals(FLASE)) {
                System.out.println(otherPlayerColor
                        + " player rejects your request for draw");
                draw = false;
                break;
            }
            if (toLowerCase.equals(TRUE)) {
                System.out.println(otherPlayerColor
                        + " player confirms your request for draw");
                draw = true;
                break;
            }
    ...
    public Move askForMove() {
        ...
        while (true) {
            ...
            System.out.println(
                    this.name + " please make a move:  For example A2 A3");
